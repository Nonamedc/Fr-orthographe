...
function srsDueErrors(){
  const p=curP();if(!p||!p.errors)return[];
  return p.errors.filter(e=>!e.srs_next||e.srs_next<=Date.now());
}
...
function openProfileScreen(idx){
  ...
  changeBtn.className='ps-av-change-btn';
  changeBtn.textContent='✏️ Changer';
  changeBtn.style.cssText='position:absolute;bottom:-22px;left:50%;transform:translateX(-50%);background:var(--bg3);border:1.5px solid var(--bg4);border-radius:8px;padding:3px 10px;font-size:.65rem;font-weight:700;color:var(--txt2);cursor:pointer;white-space:nowrap;z-index:2';
...
DB.p.forEach(p=>{
  ...
  p.errors.forEach(e=>{
    ...
    if(e.t==='nature'&&!e.mot) e._invalid=true; // marquer invalides
  });
});
saveDB();
...
function starsEarned(xp){return Math.max(1,Math.floor(xp/10));}
